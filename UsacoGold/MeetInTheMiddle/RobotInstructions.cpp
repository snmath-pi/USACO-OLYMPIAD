...
int main() {
	...
	vpll b(begin(a), begin(a)+n/2);
	vpll c(begin(a)+n/2, end(a));
	debug(b) debug(c)

	unordered_map<pll, unordered_map<ll, ll>, hsh> msk;
	vl res(n+1);

	auto fun = [&](vpll &X, int val) {
		...
		rep(i,1<<m) {
			...
			if(val) {
				ll xj = xg-xi, yj = yg-yi;
				if(msk.count({xj, yj})) {
					for(auto u: msk[{xj, yj}]) res[cnt+u.fi]+=u.se;
				}
