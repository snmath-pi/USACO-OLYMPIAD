...
int main() {
    ...
    ll M = (n + m - 2) / 2;
    unordered_map<pll, unordered_map<ll, ll>, hsh> mp;


    auto dfs = [&](ll x, ll y, ll val, ll move, auto &&dfs)->void {
    	if(!(x>=1&&y>=1&&x<=n&&y<=m)) return;
    	if(move == M) {
    		debug(val)
    		debug(mpr(x, y))
    		mp[{x, y}][val^a[x][y]]++;return;
    	}
    	val^=a[x+1][y];
    	dfs(x+1,y,val,move+1,dfs);
    	val^=a[x+1][y];
    	val^=a[x][y+1];
    	dfs(x,y+1,val,move+1,dfs);
    	val^=a[x][y+1];
    }; dfs(1, 1, a[1][1], 0,dfs);

    ll res = 0;
    auto dfs2 = [&](ll x, ll y, ll val, ll move, auto &&dfs2)->void {
    	if(!(x>=1&&y>=1&&x<=n&&y<=m))return;
    	if(move == n + m - 2 - M) {
    		debug(val)
    		debug(mpr(x, y))
    		if(mp[{x, y}].count(val^k)) {
    			res += mp[{x,y}][val^k];
    		}
    		return;
    	}
    	val^=a[x-1][y];
    	dfs2(x-1,y,val,move+1,dfs2);
    	val^=a[x-1][y];
    	val^=a[x][y-1];
    	dfs2(x,y-1,val,move+1,dfs2);
    	val^=a[x][y-1];

    }; dfs2(n,m,a[n][m],0,dfs2);
