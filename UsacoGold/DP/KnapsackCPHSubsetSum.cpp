...
int main() {
    ...
    cin.tie(0);
    // what are all the sums possible using given coins
    int n; 
    cin >> n;
    vi a(n + 1);
    repn(i, n) cin >> a[i];
    ...
    debug(sum);
    vvi possible(sum + 1, vi(n + 1, 0));
    possible[0][0] = 1;
    repn(i, n) {
        rep(j, sum + 1) {
            if(j - a[i] >= 0) possible[j][i] |= possible[j-a[i]][i-1];
    ...
    rep(i, sum + 1) {
        repn(j, n) {
            if(possible[i][j]) res.insert(i);
        }
    }
    debug(res);
    for(auto x: res) cout << x << sp;

    cout << "Can we make sum using all ?";
    cout << possible[sum][n] << nl;
    //using O(n) space
    vi can(sum + 1, 0);
    can[0] = 1;
    repn(i, n) {
